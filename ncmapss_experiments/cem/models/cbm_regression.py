...
from cem.metrics.accs import compute_metric
...
class ConceptBottleneckModel(pl.LightningModule):
    def __init__(
        ...
        if exclusive_concepts:
            self.loss_concept = torch.nn.CrossEntropyLoss(weight=weight_loss)
        else:
    ...
    def _run_step(
        ...
        # compute accuracy
        (c_accuracy), (y_accuracy) = compute_metric(
            c_sem,
            y_logits,
            c,
            y,
            num_labels=self.n_concepts
        )
        '''
        result = {
        ...
        result = {
            ...
            "concept_loss": concept_loss_scalar,
            "task_loss": task_loss_scalar,
            "rmse_unscaled": np.sqrt(task_loss_scalar.cpu().detach())*100,
            "loss": loss.detach(),
        }
    ...
    def training_step(self, batch, batch_no):
        ...
        for name, val in result.items():
            ...
            self.log(name, val, prog_bar=prog_bar, on_step=True, on_epoch=True)
        return {
            "loss": loss,
            "log": {
    ...
    def validation_step(self, batch, batch_no):
        ...
        for name, val in result.items():
            if self.n_tasks <= 2:
                prog_bar = (("loss" in name))
            else:
                prog_bar = (("c_auc" in name) or ("y_accuracy" in name))
            self.log("val_" + name, val, prog_bar=prog_bar, on_step=False, on_epoch=True)
        result = {
            "val_" + key: val
