...
## HELPER FUNCTIONS
################################################################################
def compute_bin_metric(c_pred, y_pred, c_true, y_true, num_labels):
    if num_labels > 1:
        metric = MultilabelAccuracy(num_labels=num_labels).to(device)
    else:
        metric = BinaryAccuracy().to(device)
    c_accuracy = metric(c_pred, c_true)
    mean_squared_error = MeanSquaredError(num_outputs=1).to(device)
    y_mse = mean_squared_error(torch.squeeze(y_pred), y_true)
    '''
    ...
    c_f1 = c_f1/num_seen
    y_accuracy = sklearn.metrics.mean_squared_error(y_true, y_pred)
    ...
    '''
    return (c_accuracy), (y_mse) #(c_accuracy, c_auc, c_f1), (y_accuracy, 0, 0)
